...
/* ---------- LISTA ALFABETICA (A-Z) ---------- */
function renderListPage(){
  const app = document.getElementById('main-content');
  if(!app) return;

  // costruisce layout base: indice lettere + contenuto
  app.innerHTML = `
    <section class="section list-page" id="alpha-list">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        <h2>Elenco alfabetico</h2>
        <div style="display:flex;gap:8px;align-items:center;">
          <input id="listSearchBox" placeholder="Cerca nell'elenco..." style="padding:8px;border-radius:6px;background:#0f0f10;color:#eee;border:1px solid #222">
          <select id="listTypeFilter"><option value="all">Tutti</option><option value="film">Film</option><option value="serie">Serie</option></select>
        </div>
      </div>

      <div id="alphaIndex" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;"></div>

      <div id="alphaContent" class="alpha-content"></div>
    </section>
  `;
  ...
  // applica filtri e ri-crea la mappa
  function buildMapAndRender(){
    const type = document.getElementById('listTypeFilter')?.value || 'all';
    const q = (document.getElementById('listSearchBox')?.value || '').trim().toLowerCase();

    let pool = items.filter(it => {
      if(type === 'film' && it.tipo !== 'film') return false;
      if(type === 'serie' && it.tipo !== 'serie') return false;
      if(q){
        const inTitle = String(it.titolo||'').toLowerCase().includes(q);
        const inPlot  = String(it.trama||'').toLowerCase().includes(q);
        if(!(inTitle || inPlot)) return false;
      }
      return true;
    });

    // ordina alfabeticamente per titolo (locale, sensibile agli accenti)
    pool.sort((a,b) => a.titolo.localeCompare(b.titolo, 'it', {sensitivity:'base'}));

    const map = {};
    pool.forEach(it => {
      const L = initialLetter(it.titolo);
      if(!map[L]) map[L] = [];
      map[L].push(it);
    });
    ...
    renderAlphaContent(map);
  }

  function renderAlphaIndex(letters){
    const idx = document.getElementById('alphaIndex');
    idx.innerHTML = '';
    ...
    order.forEach(ch => {
      ...
      if(present.has(ch)) {
        btn.addEventListener('click', ()=> {
          const el = document.getElementById(`alpha-section-${ch}`);
  ...
  function renderAlphaContent(map){
    ...
    if(orderedKeys.length === 0){
      container.innerHTML = '<div style="color:#bbb;padding:16px">Nessun elemento</div>';
      return;
    }
    orderedKeys.forEach(letter => {
      ...
      const list = document.createElement('div'); list.className = 'alpha-list-grid';
      map[letter].forEach(it => {
        const row = document.createElement('div');
        row.className = 'alpha-item';
        ...
        const img = document.createElement('img'); img.src = it.locandina || FALLBACK_POSTER; img.loading='lazy';
        img.style.width = '54px'; img.style.height = '76px'; img.style.objectFit = 'cover'; img.style.borderRadius = '4px';
        img.addEventListener('error', ()=> img.src = FALLBACK_POSTER);
        const meta = document.createElement('div');
        meta.innerHTML = `<strong style="display:block">${escapeHtml(it.titolo)}</strong><span style="font-size:12px;color:#bbb">${it.anno ? it.anno + ' · ' : ''}${(it.generi||[]).slice(0,2).join(', ')}</span>`;
        ...
        openBtn.addEventListener('click', (ev) => { ev.stopPropagation(); openModal(it); });
  ...
  // attach events
  const sb = document.getElementById('listSearchBox');
  const tf = document.getElementById('listTypeFilter');
  if(sb) sb.addEventListener('input', ()=> { if(window._deb) clearTimeout(window._deb); window._deb = setTimeout(buildMapAndRender, 200); });
  if(tf) tf.addEventListener('change', buildMapAndRender);
